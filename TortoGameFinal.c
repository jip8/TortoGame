...
int VerificaPalavra(char matriz[6][3],char str[18]) {
  int pos;
  int teste[6][3];
  char strAux[18];
  for(int i = 0;i<6;i++){
...
int main() {
  ...
  Cont=0;
  char palavras[100][18];
  
  printf("\n  !!! BEM VINDO AO JOGO TORTO !!!");
  while (flag){
    printf("\n  Ja foram encontradas %i palavras:\n", Cont);
    ...
    if (len==0){
      ...
      printf("\n  Esta palavra ja foi encontrada!");
  }  
  return 0;
}